...
def loadTrial(trialPath):
    ...
    try:
        if isinstance(list(trialDict['finalPatches'].values())[0], dict):
            trial.finalPatches = {}
            for area, patchDict in list(trialDict['finalPatches'].items()):
                try:
    ...
    try:
        if isinstance(list(trialDict['finalPatchesMarked'].values())[0], dict):
            ...
            for area, patchDict in list(trialDict['finalPatchesMarked'].items()):
                try:
...
def plotPatches(patches, plotaxis=None, zoom=1, alpha=0.5, markersize=5):
    ...
    imageHandle = {}
    for key, value in list(patches.items()):
...
def plotPatchBorders3(patches, altPosMap, aziPosMap, plotAxis=None, plotSize=None, borderWidth=2, zoom=1,
    ...
    for key, currPatch in list(patches.items()):
...
class RetinotopicMappingTrial(object):
    ...
    def _getSignMap(self, isReverse=False, isPlot=False, isFixedRange=False):
    ...
    def _getRawPatches(self, isPlot=False):
        ...
        for key, value in list(rawPatches2.items()):
            if (value.getArea() < smallPatchThr):
                rawPatches.pop(key)

        # remove isolated Patches
        rawPatches2 = dict(rawPatches)
        for key in list(rawPatches2.keys()):
            isTouching = 0
            for key2 in list(rawPatches2.keys()):
                if key != key2:
    ...
    def _getDeterminantMap(self, isPlot=False,isFixedRange=False):
            ...
            if AS / AU >= splitOverlapThr:
                ...
                if NumOfMin == 1:
                    ...
                    if len(newPatches) > 1:
                        ...
                        currPatchValue = 0
                        for key2, value2 in list(newPatches.items()):
                            currPatchValue += 1
    ...
    def _mergePatches(self, isPlot=False):
        ...
        for key, value in list(patches2.items()):
            if (value.getArea() < smallPatchThr):
                patches.pop(key)
    ...
    def generateTrialDict(self,
        ...
        for key in list(self.__dict__.keys()):
            ...
            if key in keysToRetain:
                if key == 'finalPatches':
                    finalPatches = {}
                    for area, patch in list(self.finalPatches.items()):
                    ...
                    finalPatchesMarked = {}
                    for area, patch in list(self.finalPathcesMarked.items()):
    ...
    def normalize(self, centerPatchKey='patch01', mapSize=800, isPlot=False, borderValue=0.):
        ...
        patchesNor = {}
        for key, patch in list(patches.items()):
    ...
    def plotNormalizedPatchCenter(self, centerPatchKey='patch01', mapSize=512, plotAxis=None, markerSize=5.,
        ...
        for key, patch in list(self.finalPatches.items()):
    ...
    def plotTrial(self, isSave=False, saveFolder=None):
        ...
        for key, value in list(self.rawPatches.items()):
    ...
    def plotFinalPatchBorders(self, plotAxis=None, plotName=True, plotVasMap=True, isTitle=True, isColor=True,
        ...
        for key, patch in list(finalPatches.items()):
            mask = patch.getMask()
            if isColor:
                if patch.sign == 1:
                    plotColor = '#ff0000'
    ...
    def _generateTotalMask(self):
        ...
        for patch in list(self.finalPatches.values()):
    ...
    def plotPatchesWithName(self, patchDict, plotAxis=None):
        ...
        for key, patch in list(patchesForPlotting.items()):
            center = patch.getCenter()
    ...
    def plotContours(self, isNormalize=True, altLevels=np.arange(-30., 50., 5.), aziLevels=np.arange(0., 120., 5.),
        ...
        if hasattr(self, 'vasculatureMap') and not isinstance(self.vasculatureMap, type(None)) and isPlottingBorder:
